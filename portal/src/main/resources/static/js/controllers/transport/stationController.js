angular.module('srmt').controller(
		"stationController",
		function($scope, $state, stationService, $stateParams, controlService,
				$localStorage, toaster) {

			$scope.gotoStationsList = function() {
				$state.go('home.station.stationList');
			}

			$scope.gotoAddStationsList = function() {
				$state.go('home.station.addStation');
			}
